with open("D:/AcaDesk1/ui/dashboard.py", "r", encoding="utf-8") as f:
    lines = f.readlines()
...
for i, line in enumerate(lines):
    if line.startswith("    def show_student_list(self):"):
        start_idx = i
    if line.startswith("    def open_deep_analysis(self, data, report):"):
        end_idx = i
        break

if start_idx != -1 and end_idx != -1:
    new_code = """    def show_student_list(self):
        self._clear()
        ...
        lbl_title = ctk.CTkLabel(left_tool, text=f"{name} > {self.current_year}", font=FONTS["h2"], text_color="white")
    ...
    def _draw_modern_row(self, s, report):
        ...
        if len(name_parts) >= 2: initials = (name_parts[0][0] + name_parts[1][0]).upper()
        elif len(name_parts) == 1 and len(name_parts[0]) > 0: initials = name_parts[0][0].upper()
        else: initials = "?"
            
        initials_lbl = ctk.CTkLabel(avatar_f, text=initials, font=("Arial", 14, "bold"), text_color="white")
        ...
        lbl_name = ctk.CTkLabel(name_f, text=name_str, font=("Arial", 14, "bold"), text_color="white")
        ...
        score_pill.pack_propagate(False)
        ...
        conf_pill.pack_propagate(False)
        ...
        if tot > 0 or st != 'NONE':
            ...
            note_pill.pack_propagate(False)
...
"""
    ...
    with open("D:/AcaDesk1/ui/dashboard.py", "w", encoding="utf-8") as f:
        f.writelines(lines)
    print("Dashboard UI patched successfully!")
else:
